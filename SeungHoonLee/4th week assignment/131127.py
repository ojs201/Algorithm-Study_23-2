...
def solution(want, number, discount) -> bool:
    return sum(is_products_discounted(day, want, number, discount[day:day + 10]) for day in range(len(discount)))


def is_products_discounted(day, want, number, discount) -> bool:
